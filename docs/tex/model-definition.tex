...
Let a supernova be observed in $n$ photometric bands and write its magnitude vector
...
extinction but not K-corrected; only the phase argument is rest-frame. The
distinction is not pedantry: recasting the $m_{X}$ as rest-frame quantities would
require an SED, which this model does not have, and the $\zred < 0.05$ cut of
Section~\ref{sec:samples} exists to keep the resulting mismatch small rather than to
remove it.
...
space because the nuisances that matter act there as translations, to which curvature
...
departure from the instantaneous osculating plane. With two bands there is no
torsion, so in the primary sample $\shapetwo$ is a second direction of variation in
$\curv$ and must not be called torsion.

The invariants are represented by a neural network conditioned on per-supernova
latents,
\begin{equation}
...
shape and phase are learned rather than imposed. The cost is that $\shapeone$ and
$\shapetwo$ carry no a~priori reading as ``amount of component one''; their meaning
is established after training by correlating them against observables.
...
curvature without altering the net total. Two corollaries: $\curv \to 0$ at both
...
\paragraph{Position.} Two components, fixed differently. Perpendicular to $(1,1)$ the
offset of the early ray from the origin \emph{is} the early colour, a template
function of $\shape$; being a property of the template it costs no per-supernova
parameter. Parallel to $(1,1)$ the template is normalised so that $\magg = 0$ at its
own $g$-maximum, so that the per-supernova offset $\dm$ of \eqref{eq:forward} is
exactly the peak $g$ magnitude. That normalisation is a deterministic function of
...
$\norm{\dd\curve/\dd\phase} = 0$, hence $\dd\arc/\dd t = 0$, the positivity requirement
of \eqref{eq:reparam} is violated, and the unit-speed parameterisation fails: the curve
has a stationary rather than a regular point. Non-coincident band maxima is thus a
...
\begin{equation}
  ...
  \label{eq:dustcurve}
\end{equation}
with $\dustdir$ precomputed once from an extinction law and a template SED and its
amplitude $\colr$ free per supernova. That $\dustdir$ \emph{must} depend on $\arc$, and
what follows from it, is Section~\ref{sec:phasedust}; here it is enough that being
phase-dependent it cannot be folded into $\dm$ after the fact. The term appears only in
rung L2c; below that rung $\colr = 0$ and reddening is left to the shape latents, which
is the failure the rung tests for.
...
Seven per supernova, each belonging to one layer above:
\begin{center}
\begin{tabular}{llll}
\toprule
...
Placement (\ref{sec:place})   & \emph{none}            & ---   & fixed by the boundary \\
...
\midrule
Rung L2c only                 & $\colr$                & reddening amplitude & deformation \\
\bottomrule
\end{tabular}
\end{center}
That placement costs nothing is worth stating outright: orientation and position come
from \eqref{eq:frameanchor2} and the normalisation of Section~\ref{sec:place}, both
template properties. $\tmax$ enters the traversal mechanically through
\eqref{eq:reparam} but is interpreted as an observable. $\colr$ is not a motion of the
ambient space at all: it scales $\dustdir(\arc)$, which moves different points of the
curve by different amounts (Section~\ref{sec:phasedust}).
...
introduced one at a time along a ladder from a rigid template (L0: $\dm, \tscale,
\tmax$) to the full model, each rung scored on held-out Hubble residual scatter under
cross-validation and never on in-sample fit, with \salt\ fitted independently on the
same objects as an external benchmark. The ladder, which is also the training
schedule, is specified in the project's engineering record rather than here.
...
Terminal colour                  & prediction & asymptotic ray separation, \eqref{eq:sumrule} \\
...
initialise, or K-correct, so independence from \salt\ is preserved. The discussion
...
The decisive structural difference is the treatment of timing. \salt\ has a single time
parameter and forces all remaining temporal variation into the shape components; the
geometric model gives timing its own map and can therefore ask whether shape variation
is real or merely mis-assigned timing.
...
\begin{equation}
  ...
  \label{eq:tripp}
\end{equation}
with $\alpha$, $\beta$ and $\mathcal{M}$ fitted globally. This is the same logical
structure as \salt\ cosmology and makes the comparison direct. It is also why the
