...
The domain is semi-infinite in arclength, and over its early part the curvature is set
to zero rather than learned,
\begin{equation}
  \arc \in (-\infty, \arc_{\mathrm{end}}], \qquad
  \curv_{i}(\arc; \shape) = 0 \quad \text{for } \arc \le 0 ,
  \label{eq:straight}
\end{equation}
so the curve begins as a straight ray. The motivation is the
pre-explosion state: a supernova has no flux before it explodes, and zero flux in every
band means $m \to +\infty$, infinitely far out along the ray. The domain must reach that
state, so it extends without bound; and the featureless approach to it has no shape to
learn, so its curvature vanishes. There the frame is constant and the predicted flux is
zero in all bands, for any $n$; the normal stays well defined because \eqref{eq:frenet2d}
fixes $\norml$ as the $90^{\circ}$ rotation of $\tang$, not as $\curve''/\norm{\curve''}$,
which is undefined where $\curv = 0$. For $n \ge 3$ the torsion vanishes by the same
construction, and that it cannot be recovered from a featureless ray costs nothing, since
the prediction does not depend on it.

\subsection{Where the curve sits}
\label{sec:place}

A supernova's curvature and its placement in $\Rn$ are both fitted. The curvature is the
invariants \eqref{eq:kappanet}; the placement is a rigid motion, and it enters in two
ways. Its rotation --- the frame orientation --- is a single global parameter, determined
by the fit and shared across the whole sample. Its translation is fixed for the template
by convention (the arclength origin and $\magg = 0$ at the template $g$-maximum, both
below), and each supernova then carries two parameters, $\dm$ along $(1,1)$ and $\colr$
along $\hat{\mathbf{e}}_{c}$, that translate it from that reference. The two directions
are independent, so $(\dm, \colr)$ span the translation freedom in full and no third
per-supernova placement parameter is needed.

The zero-curvature segment \eqref{eq:straight} does not itself place the curve ---
position is the convention together with the fitted $(\dm, \colr)$, and orientation is
the fitted $\Phi$. Its role in placement is narrower: it makes the arclength origin well
defined, at the end of the segment, and fixing $\arc = 0$ there removes the flat
direction the origin would otherwise share with the translations. What the pre-explosion
state supplies is the boundary condition of the next paragraphs, not the location.

In every band $f \to 0$ at both extremes, hence $m \to +\infty$: both ends escape to
infinity. This much is forced by the photometry; the direction of escape is not, since
it depends on the relative rates at which the two bands vanish. The curve is therefore a
hairpin rather than an arc --- both ends run off toward $m = +\infty$ in every band, and
the tangent reverses between them, turning through approximately $\pi$ over the life of
the supernova. Two things follow that do not depend on the escape direction: the
curvature tends to zero at both extremes as the curve straightens onto its asymptote,
which is \eqref{eq:straight}, and the offset between the two asymptotic rays is the
terminal colour, an output of the model rather than an input. The total turning
$\int \curv\,\dd\arc \approx \pi$ is a qualitative diagnostic only. Its exact value is
not pinned: the early orientation is fitted (below), and the late asymptote lies beyond
the model domain $(-\infty, \arc_{\mathrm{end}}]$ of \eqref{eq:straight} and is never
observed. It is never imposed as a constraint.

The two ends are not points of $\Rn$,
...
The initial frame is set by a single global orientation parameter $\Phi$, common to all
supernovae rather than fitted per object,
\begin{equation}
  \tang(\arc) = R(\Phi)\left(-\tfrac{1}{\sqrt{2}}(1,1)\right) \quad \text{for } \arc \le 0 ,
  \label{eq:frameanchor2}
\end{equation}
with $R(\Phi)$ the rotation by $\Phi$, constant on the segment by \eqref{eq:straight}.
At $\Phi = 0$ the early ray lies along the diagonal, $\dd(\magg - \magr)/\dd\arc = 0$:
the two bands share one early power-law index (Section~\ref{sec:traversal}) and the
colour is constant throughout the earliest modelled phase. That constant-early-colour
condition was previously imposed; here it is the nested null $\Phi = 0$ of a fitted
quantity, so the early colour behaviour is measured rather than assumed.

The orientation is the one rigid degree of freedom the geometry leaves free. A rigid
rotation is not absorbed by the per-supernova translations $\dm$ and $\colr$, and
$\curv(\arc; \shape)$ is rotation-invariant, so the template's orientation in the plane
is fixed by nothing but $\Phi$. This is why it is a parameter and not a gauge: rotating
the template changes the predicted magnitudes and cannot be undone downstream. And being
a single number shared across the sample it is well determined by the population
colour--phase relation, which is densely sampled near maximum, even though each
supernova's early ray individually is not --- so it is constrained where the data are
strongest, not where they are sparse.

For $n=3$ the orientation is an element of $\SO(3)$, three parameters, and all three are
fitted globally in the same way. No residual frame condition is left open, in contrast
to fixing a direction, which would supply only two of the three and leave the rotation
about the diagonal axis to be recovered from an observable.

The curve carries two per-supernova translations --- position along two directions the
geometry treats quite differently, both phase-independent shifts to which $\curv$ is
blind by \eqref{eq:transinv}. Along $(1,1)$ lies distance: luminosity distance,
absolute luminosity and peculiar velocity each shift every band by the \emph{same}
amount, so this direction is fixed by physics rather than fitted. The template is
normalised so that $\magg = 0$ at its own $g$-maximum --- a deterministic function of
$\shape$, not a parameter --- so the per-supernova offset $\dm$ of \eqref{eq:forward}
along $(1,1)$ is then exactly the peak $g$ magnitude. The second direction is
reddening. Which direction that is --- the reddening vector, set by the extinction law
and the mean SED --- is not fixed by any per-supernova physics, so rather than assume it
the model fits it as a single global unit vector $\hat{\mathbf{e}}_{c}$, common to all
supernovae exactly as the orientation $\Phi$ is; this is the two-band analogue of \salt\
fitting $\beta$ rather than adopting a value of $R_{V}$, with the fixed-$R_{V}$
idealisation $\hat{\mathbf{e}}_{c} = (1,-1)/\sqrt{2}$ of \eqref{eq:transcol} the nested
case. The per-supernova offset along
$\hat{\mathbf{e}}_{c}$ is the colour $\colr$, the direct counterpart of \salt's colour
parameter, absorbing host reddening and intrinsic colour scatter together.
...
translating the arclength origin displaces the curve \emph{along} the early ray,
$R(\Phi)(1,1)$ --- a direction that reduces to $\dm$'s own axis $(1,1)$ at $\Phi = 0$
and otherwise mixes $\dm$ with $\colr$. Either way the arclength origin and the
per-supernova translations span an exactly flat direction of the likelihood if the
origin is left unfixed. Anchoring $\arc = 0$ at the end of the segment, as in
\eqref{eq:straight}, removes it.

Because the frame is anchored on the early segment rather than at maximum, the
$g$-maximum is not at $\arc = 0$ but is located on the template as the first root
\begin{equation}
  \deriv{\magg}{\arc}\bigg|_{\arc = \arc_{g}(\shape)} = 0 ,
  \label{eq:frameanchor}
\end{equation}
a $\shape$-dependent root differentiable through the implicit function theorem; the
traversal of Section~\ref{sec:traversal} maps $\tmax$ onto this $\arc_{g}(\shape)$. One
regularity condition on the placement remains.
...
The logarithmic term is forced by \eqref{eq:unitspeed}: exactly zero predicted flux requires
...
The straight segment and the singular map together predict a power law. On the early
ray $m_{X} = c_{X} - e_{X}\arc$, where $(e_{g}, e_{r}) = R(\Phi)(1,1)/\sqrt{2}$ is the
unit ray direction of \eqref{eq:frameanchor2}, and near $t_{\mathrm{expl}}$ the $\ln x$
term dominates \eqref{eq:reparam}, so
\begin{equation}
  f_{X} \propto \bigl(t - t_{\mathrm{expl}}\bigr)^{\alpha_{X}},
  \qquad
  \alpha_{X} = 0.4\ln 10 \; e_{X}\,A ,
  \label{eq:powerlaw}
\end{equation}
a power-law rise in each band. The two indices share the scale $A$ and differ only
through the orientation: $\alpha_{g} = \alpha_{r}$ precisely when $\Phi = 0$, where
$e_{X} = 1/\sqrt{2}$ and $\alpha = 0.651\,A$, a fireball $\alpha = 2$ corresponding to
$A \simeq 3.07$. So \eqref{eq:straight} is not an architectural device but a statement
with an observational counterpart, and $A$, like $u_{\mathrm{expl}}$ and the window
below, is a template hyperparameter held in configuration so its sensitivity can be
tested. The ratio $\alpha_{g}/\alpha_{r}$ is the observable form of $\Phi$: equal
indices confirm the diagonal early ray, and a fitted $\Phi \neq 0$ predicts a definite
inequality between the two bands' rise indices.
...
The rest-frame window must be distinguished into three objects that are easily conflated.
\begin{center}
\begin{tabular}{ll}
\toprule
Object & Extent \\
...
\end{tabular}
\end{center}
Placement costs exactly one per-supernova parameter, and only in position: both
directional freedoms --- the orientation $\Phi$ of \eqref{eq:frameanchor2} and the
reddening direction $\hat{\mathbf{e}}_{c}$ of Section~\ref{sec:place} --- are single
global parameters shared across the sample, not per-supernova costs. The component of
position along $(1,1)$ is fixed by the normalisation of Section~\ref{sec:place}, while
the component along $\hat{\mathbf{e}}_{c}$ carries $\colr$. $\tmax$ enters the traversal
...
Reddening direction $\hat{\mathbf{e}}_{c}$ & \emph{parameter} & global, one per sample; \\
                                 &            & $(1,-1)/\sqrt{2}$ is the fixed-$R_{V}$ idealisation \\
Frame orientation $\Phi$         & \emph{parameter} & global, one per sample \eqref{eq:frameanchor2}; \\
                                 &            & $\Phi = 0$ is constant early colour \\
...
condition of the form $\dd\magg/\dd\arc = 0$ only labels a point of the template.
Orientation is instead carried by the fitted global $\Phi$: the diagonal direction is
physically distinguished in magnitude space, so the early ray's orientation is a claim
about early colour evolution, and rather than assert it the model fits one number for
the whole sample and reads $\Phi = 0$ off as the nested constant-early-colour case. That
one parameter also fixes the frame completely for $n=3$, where fixing a direction would
have left a rotation open.
...
\eqref{eq:transinv} --- so a single rotation shared across the sample, the $\Phi$ of
Section~\ref{sec:place}, is fitted. That is not a per-supernova freedom and does not
reintroduce one. The reddening direction $\hat{\mathbf{e}}_{c}$ is global for the same
reason: a per-supernova reddening direction would rotate the colour axis differently
for each object with no physical referent, so only its sample-wide value is fitted.
...
afterthought. By \eqref{eq:straight} the torsion vanishes on the zero-curvature segment
(Section~\ref{sec:curve}), so it is constrained only where the track turns.
